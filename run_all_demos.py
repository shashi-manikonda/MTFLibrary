...
def run_demos():
    ...
    Keeps all temporary files in runoutput.
    """
    ...
    for root, _, files in os.walk(demos_dir):
        ...
        for file in files:
            ...
            try:
                ...
                # Create a wrapper script that configures matplotlib
                # and runs the demo
                ...
                if result.returncode == 0:
                    ...
                    if result.stdout.strip():
                        ...
                        if len(stdout_lines) > 3:
                            print(
                                f"      ... ({len(stdout_lines) - 3} "
                                "more lines in output file)"
                            )
                else:
                    failed_demos += 1
                    print(
                        f"    ✗ FAILED - {demo_duration:.2f}s "
                    ...
                    if result.stderr:
                        ...
                        print(
                            "      Error: "
                            f"{error_lines[-1] if error_lines else 'Unknown'}"
                        )
...
def move_temp_files_to_output(script_path, wrapper_script, demo_output_dir, demo_name):
    ...
    try:
        ...
        if script_path and os.path.exists(script_path):
            converted_py_dest = os.path.join(
                demo_output_dir, f"{demo_name}_converted.py"
            )
