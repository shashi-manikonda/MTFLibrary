...
import pytest
import numpy as np
from MTFLibrary.taylor_function import initialize_mtf_globals, get_global_max_order, get_global_max_dimension, set_global_max_order, set_global_etol, get_global_etol, Var, MultivariateTaylorFunction, ComplexMultivariateTaylorFunction, convert_to_mtf
...
def test_global_initialization(setup_function):
    ...
    assert get_global_max_dimension() == 6
    with pytest.raises(RuntimeError):
        initialize_mtf_globals(max_order=6, max_dimension=3) # Re-initialization should fail
...
def test_mtf_get_min_coefficient(setup_function):
    ...
    coeffs_negligible = {(0, 0): np.array([1e-10]).reshape(1)}
...
def test_mtf_power(setup_function):
    ...
    with pytest.raises(ValueError):
        mtf ** (-1) # Negative power not allowed
    with pytest.raises(ValueError):
...
def test_elementary_functions_scalar(setup_function, func, func_name):
    ...
    assert isinstance(mtf_result, MultivariateTaylorFunction)
    scalar_eval_result = func(scalar_input).eval([0]) # Evaluate at 0 as Taylor series is around 0
    ...
    elif func_name == 'sqrt_taylor': expected_val = np.sqrt(1 + scalar_input)
    elif func_name == 'log_taylor': expected_val = np.log(1 + scalar_input)
    ...
    assert np.allclose(scalar_eval_result, np.array([expected_val]).reshape(1), rtol=1e-1)
