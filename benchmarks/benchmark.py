...
# This is a temporary fix to allow the benchmark to be run from the
...
from mtflib.taylor_function import (  # noqa: E402
    MultivariateTaylorFunction,
)
from src.applications.em.biot_savart import serial_biot_savart  # noqa: E402
...
def generate_random_mtf(dimension, max_order, num_terms):
    ...
    This avoids generating all possible exponents to save memory.
    """
    if dimension == 0:
...
def benchmark_arithmetic(mtf1, mtf2, implementation="python"):
    ...
    print(
        f"\n--- Benchmarking Arithmetic Operations (Implementation: "
        f"{implementation}) ---"
    )
    ...
    for _ in range(ADD_ITERATIONS):
        _ = mtf1 + mtf2
    end_time = time.time()
    print(f"Time for {ADD_ITERATIONS} additions: "
    ...
    end_time = time.time()
    print(
        f"Time for {MUL_ITERATIONS} multiplications: "
        f"{end_time - start_time:.6f} seconds"
    )
    ...
    end_time = time.time()
    print(
        f"Time for {POW_ITERATIONS} power ops (n={POW_EXPONENT}): "
        f"{end_time - start_time:.6f} seconds"
    )
...
def benchmark_biot_savart(implementation, num_points):
    ...
    print(
        f"\n--- Benchmarking serial_biot_savart "
    ...
    end_time = time.time()
    print(
        f"Time for serial_biot_savart with {num_points} points: "
        f"{end_time - start_time:.6f} seconds"
    )
...
def run_benchmarks(args):
    ...
    setup_environment(args.order, args.dimension,
                      DEFAULT_ETOL, args.implementation)
    print(
        f"Benchmark Configuration: Order={args.order}, "
        f"Dimension={args.dimension}, "
        f"Terms={args.num_terms}, "
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Benchmark script for mtflib."
    )
    parser.add_argument(
        "--order",
        type=int,
        default=DEFAULT_ORDER,
        help="Maximum order of Taylor series.",
    )
    parser.add_argument(
        "--dimension",
        type=int,
    ...
    parser.add_argument(
        "--num-terms",
        type=int,
        default=DEFAULT_NUM_TERMS,
        help="Number of non-zero terms in the random MTFs.",
    )
    parser.add_argument(
        "--num-points",
        type=int,
        default=DEFAULT_NUM_POINTS,
        help="Number of field points for Biot-Savart benchmark.",
    )
    parser.add_argument(
        "--implementation",
        type=str,
    ...
    if args.profile:
        ...
        stats_file = os.path.join(
            profiling_dir,
            f"profile_results_{args.implementation}_order{args.order}_"
            f"dim{args.dimension}_terms{args.num_terms}.prof",
        )
